...
say "Agent Valley · the Buildyard · setup"
...
# The Buildyard is Gemini-on-Vertex and nothing else: no BigQuery, no Cloud Run,
# no Agent Engine. The crew runs in-process and the yard is a local Next.js app,
# so it is still one API and one enable. It is idempotent, so re-runs are free —
# and if you did week one on this project, it is already on.
say "1 · Vertex AI API"
...
# The yard itself is a Next.js app, and `bash valley.sh` will want npm. Nothing
...
if ! command -v npm >/dev/null 2>&1; then
    warn "npm not found. The agent side works without it, but site/ (the yard)"
