...
if [ -f yard/service.py ]; then
  .venv/bin/uvicorn yard.service:app --port "$AGENT_PORT" --log-level warning &
else
  echo "  (no yard/service.py yet — starting the app only)"
fi
(cd site && npm run dev -- --port "$APP_PORT") &

echo
echo "  yard   → http://127.0.0.1:${AGENT_PORT}"
